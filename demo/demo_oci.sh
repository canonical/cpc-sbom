...
sudo ${CPC_SBOM_SCRIPT_PATH} --ignore-copyright-parsing-errors --ignore-copyright-file-not-found-errors --rootdir ${ABS_MOUNT_PATH} --update-apt-cache --rootdir-architecture ${CLOUD_IMAGE_ARCHITECTURE} > "${CLOUD_IMAGE_SPDX}" 2> "${CLOUD_IMAGE_SPDX_GENERATION_LOG}" && echo "SBOM generation successfull" || echo "SBOM generation generated warnings or errors. See '${CLOUD_IMAGE_SPDX_GENERATION_LOG}' for details"
sudo ${CPC_SBOM_SCRIPT_PATH} --update-apt-cache --rootdir-architecture ${CLOUD_IMAGE_ARCHITECTURE} --ignore-copyright-parsing-errors --ignore-copyright-file-not-found-errors --include-installed-files --rootdir ${ABS_MOUNT_PATH} > "${CLOUD_IMAGE_SPDX_INCLUDING_INSTALLED_FILES}" 2> "${CLOUD_IMAGE_SPDX_GENERATION_LOG_INCLUDING_INSTALLED_FILES}" && echo "SBOM including installed files generation successfull" || echo "SBOM including installed files generation generated warnings or errors. See '${CLOUD_IMAGE_SPDX_GENERATION_LOG_INCLUDING_INSTALLED_FILES}' for details"
